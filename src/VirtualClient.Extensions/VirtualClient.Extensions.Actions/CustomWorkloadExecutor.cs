namespace CRC.VirtualClient.Extensions
{
    ...
    public class CustomWorkloadExecutor : VirtualClientComponent
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="CustomWorkloadExecutor"/> class.
        /// </summary>
        ...
        /// <summary>
        /// 'Duration' parameter defined in the workload profile.
        /// </summary>
        public TimeSpan Duration
        {
            get
            {
                // Local properties are used often to make it easier to reference parameters that were
        ...
        /// <summary>
        /// The ID to use for preserving the state of the executor.
        /// </summary>
        protected string StateId { get; } = nameof(CustomWorkloadExecutor);

        /// <summary>
        /// It is common to use local member variables or properties to keep track of the names of 
        /// supporting binaries/executables. Dependending upon the OS platform (Linux vs. Windows) we are on
        /// the names of the binaries might be different.
        /// </summary>
        protected string ConfigurationExecutablePath { get; set; }
        ...
        /// <summary>
        /// Mimics a requirement of performing some type of system configuration/settings update.
        /// </summary>
        protected Task ApplySystemSettingsAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            using (IProcessProxy process = this.processManager.CreateElevatedProcess(this.Platform, this.ConfigurationExecutablePath))
            {
                // For example only.
                ...
                // {
                //    this.Logger.LogProcessDetails<CustomWorkloadExecutor>(process, telemetryContext);
        ...
        protected override async Task ExecuteAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            if (workloadState == null || !workloadState.IsSystemConfigured)
            {
                // Mimics a requirement of performing some system settings update that requires a reboot.
                await this.ApplySystemSettingsAsync(telemetryContext, cancellationToken)
                    .ConfigureAwait(false);

                await this.stateManager.SaveStateAsync<WorkloadState>(
                    this.StateId,
                    new WorkloadState
                    {
            ...
            else
            {
                ...
                this.Logger.LogTraceMessage($"{nameof(CustomWorkloadExecutor)}.Starting", telemetryContext);

                // Dates should ALWAYS be represented in UTC.
                DateTime startTime = DateTime.UtcNow;

                string commandArguments = this.GetWorkloadCommandArguments();
                string workloadResults = await this.ExecuteWorkloadAsync(commandArguments, telemetryContext, cancellationToken)
                    .ConfigureAwait(false);

                DateTime finishTime = DateTime.UtcNow;

                await this.CaptureMetricsAsync(workloadResults, commandArguments, startTime, finishTime, telemetryContext, cancellationToken)
                    .ConfigureAwait(false);
            }
        }
        ...
        protected override async Task InitializeAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            if (this.Platform == PlatformID.Win32NT)
            {
                ...
                this.ConfigurationExecutablePath = this.Combine(this.WorkloadPackage.Path, "ConfigureSystem.exe");
            }
            else
            {
                // PlatformID.Unix
                ...
                this.ConfigurationExecutablePath = this.Combine(this.WorkloadPackage.Path, "ConfigSys");
        ...
        private Task CaptureMetricsAsync(string results, string commandArguments, DateTime startTime, DateTime endTime, EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            if (!cancellationToken.IsCancellationRequested && !string.IsNullOrWhiteSpace(results))
            {
                this.Logger.LogMessage($"{nameof(CustomWorkloadExecutor)}.CaptureMetrics", telemetryContext.Clone()
                ...
                CustomWorkloadParser resultsParser = new CustomWorkloadParser(results);
                ...
                // of data in a consistent way. The 'LogMetrics' method is an extension method to ILogger instances
        ...
        private Task<string> ExecuteWorkloadAsync(string commandArguments, EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            return this.Logger.LogMessageAsync($"{nameof(CustomWorkloadExecutor)}.ExecuteWorkload", relatedContext, async () =>
            {
                string results = null;
                ...
                using (BackgroundOperations profiling = BackgroundOperations.BeginProfiling(this, cancellationToken))
                {
                    ...
                    {
                        this.CleanupTasks.Add(() => this.processManager.SafeKill(new List<string> { "ExampleWorkload" }));
                        ...
                        if (!cancellationToken.IsCancellationRequested)
                        {
                            ...
                            await this.LogProcessDetailsAsync(workloadProcess, relatedContext, "CustomWorkload");
                            ...
                            results = workloadProcess.StandardOutput.ToString();
                        }
                    }
                }
        ...
        private string GetWorkloadCommandArguments()
        {
            return $"Workload --duration={this.Duration}";
        }

        internal class WorkloadState
        {
            [JsonProperty("isSystemConfigured", Required = Required.Always)]
            public bool IsSystemConfigured { get; set; }
        }
    }
}